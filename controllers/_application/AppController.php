...
if ($type === 'CREATE_POST') {
    ...
    if (!empty($content)) {
        ...
        if ($moderation['flagged']) {
            ...
            json_error('Bài viết của bạn vi phạm quy định: ' . implode(', ', $violations) . '. Vui lòng chỉnh sửa nội dung.');
        }
    }
...
if ($type === 'ADD_COMMENT') {
    ...
    if (!empty($content)) {
        ...
        if ($moderation['flagged']) {
            ...
            json_error('Bình luận của bạn vi phạm quy định: ' . implode(', ', $violations) . '. Vui lòng chỉnh sửa nội dung.');
        }
    }
...
if ($type === 'SEND_MESSAGE') {
    ...
    if (!empty($content)) {
        ...
        if ($moderation['flagged']) {
            ...
            json_error('Tin nhắn của bạn vi phạm quy định: ' . implode(', ', $violations) . '. Vui lòng chỉnh sửa nội dung.');
        }
    }
    ...
    if ($pusher && $message) {
        try {
            ...
            $pusher->trigger($channelName, 'message:new', [
