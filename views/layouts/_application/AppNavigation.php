...
// Hiển thị khác nhau dựa theo trạng thái đăng nhập (session email)
$__isLoggedIn = isset($_SESSION['email']) && !empty($_SESSION['email']);
$__currentEmail = $__isLoggedIn ? $_SESSION['email'] : null;
$__currentUser = null;

// Nếu đã đăng nhập và có $Vani (từ config/database.php) thì lấy user hiện tại
...
<header class="sticky top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur">
    <div class="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <div class="flex items-center gap-2">
            ...
            <?php if ($__isLoggedIn): ?>
                ...
                <button class="h-10 px-3 rounded-lg bg-vanixjnk text-white hover:bg-vanixjnk/90 transition text-sm font-medium flex items-center gap-2" type="button" onclick="toast.info('Tính năng đang phát triển', { description: 'Sắp có form đăng bài.' })">
    ...
    <!-- Horizontal nav links (secondary row for desktop) -->
